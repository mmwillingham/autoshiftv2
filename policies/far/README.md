...
## Overview
This policy installs the far-operator operator using AutoShift patterns.
...
hubClusterSets:
  hub:
    labels:
      far: 'true'
      far-subscription-name: 'far-operator'
      far-channel: 'stable'
      far-source: 'redhat-operators'
      far-source-namespace: 'openshift-marketplace'
      # far-version: 'far-operator.v1.x.x'  # Optional: pin to specific CSV version
...
```yaml
far-version: 'far-operator.v1.x.x'
```

Find available CSV versions:
```bash
# List available versions for this operator
oc get packagemanifests far-operator -o jsonpath='{.status.channels[*].currentCSV}'
```
...
## Resources
- [Operator Documentation](https://operatorhub.io/operator/far-operator) - Find your operator details
