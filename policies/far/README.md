# far AutoShift Policy

## Overview
This policy installs the fence-agents-remediation operator using AutoShift patterns.
...
hubClusterSets:
  hub:
    labels:
      ...
      # far-version: 'fence-agents-remediation.v1.x.x'  # Optional: pin to specific CSV version
...
clusters:
  my-cluster:
    labels:
      far: 'true'
      far-channel: 'fast'  # Override channel for this cluster
```
...
- **Version Pinning**: Add `far-version` label to pin to a specific CSV version
...
```yaml
far-version: 'fence-agents-remediation.v1.x.x'
```

Find available CSV versions:
```bash
# List available versions for this operator
oc get packagemanifests fence-agents-remediation -o jsonpath='{.status.channels[*].currentCSV}'
```
...
#### Common Patterns:
- `policy-far-config.yaml` - Main configuration
- `policy-far-<feature>.yaml` - Feature-specific configs

#### Template Structure:
```yaml
{{- $policyName := "policy-far-config" }}
{{- $placementName := "placement-policy-far-config" }}
...
spec:
  disabled: false
  dependencies:
    - name: policy-far-operator-install
      namespace: {{ .Values.policy_namespace }}
      apiVersion: policy.open-cluster-management.io/v1
          ...
          object-templates:
            - complianceType: musthave
              objectDefinition:
                ...
                metadata:
                  name: far-config
                  namespace: {{ .Values.far.namespace }}
                spec:
                  # Your operator-specific configuration
                  # Use dynamic labels when needed:
                  # setting: '{{ "{{hub" }} index .ManagedClusterLabels "autoshift.io/far-setting" | default "default-value" {{ "hub}}" }}'
...
# far-setting<string>: Configuration option (default: 'value')
# far-feature-enabled<bool>: Enable optional feature (default: 'false')
# far-provider<string>: Provider-specific config (default: 'generic')
...
3. Check policy status: `oc describe policy policy-far-operator-install`
...
1. Test locally: `helm template policies/far/`
2. Check hub escaping: Look for `{{ "{{hub" }} ... {{ "hub}}" }}` patterns
3. Validate YAML: `helm lint policies/far/`

## Resources
- [Operator Documentation](https://operatorhub.io/operator/fence-agents-remediation) - Find your operator details
