# machine-config-motd AutoShift Policy

## Overview
This policy installs the machine-config-motd operator using AutoShift patterns.
...
helm template policies/machine-config-motd/
```
...
hubClusterSets:
  hub:
    labels:
      ...
      machine-config-motd-subscription-name: 'machine-config-motd'
      machine-config-motd-channel: 'stable'
      machine-config-motd-source: 'redhat-operators'
      machine-config-motd-source-namespace: 'openshift-marketplace'
      # machine-config-motd-version: 'machine-config-motd.v1.x.x'  # Optional: pin to specific CSV version
...
clusters:
  my-cluster:
    labels:
      machine-config-motd: 'true'
      machine-config-motd-channel: 'fast'  # Override channel for this cluster
```
...
```yaml
- name: machine-config-motd
  path: policies/machine-config-motd
  helm:
    valueFiles:
    - values.yaml
```
...
- **Version Pinning**: Add `machine-config-motd-version` label to pin to a specific CSV version
...
```yaml
machine-config-motd-version: 'machine-config-motd.v1.x.x'
```

Find available CSV versions:
```bash
# List available versions for this operator
oc get packagemanifests machine-config-motd -o jsonpath='{.status.channels[*].currentCSV}'
```
...
oc get pods -n machine-config-motd

# Check available CRDs
oc get crds | grep machine-config-motd
...
#### Common Patterns:
- `policy-machine-config-motd-config.yaml` - Main configuration
- `policy-machine-config-motd-<feature>.yaml` - Feature-specific configs

#### Template Structure:
```yaml
{{- $policyName := "policy-machine-config-motd-config" }}
{{- $placementName := "placement-policy-machine-config-motd-config" }}
...
spec:
  disabled: false
  dependencies:
    - name: policy-machine-config-motd-operator-install
      namespace: {{ .Values.policy_namespace }}
      apiVersion: policy.open-cluster-management.io/v1
  ...
  policy-templates:
    - objectDefinition:
        ...
        metadata:
          name: machine-config-motd-config
        spec:
          ...
          object-templates:
            - complianceType: musthave
              objectDefinition:
                ...
                metadata:
                  name: machine-config-motd-config
                  namespace: {{ .Values.machineConfigMotd.namespace }}
                spec:
                  # Your operator-specific configuration
                  # Use dynamic labels when needed:
                  # setting: '{{ "{{hub" }} index .ManagedClusterLabels "autoshift.io/machine-config-motd-setting" | default "default-value" {{ "hub}}" }}'
...
spec:
  ...
  predicates:
    - requiredClusterSelector:
        labelSelector:
          matchExpressions:
            - key: 'autoshift.io/machine-config-motd'
              operator: In
              values:
              - 'true'
  tolerations:
...
# machine-config-motd-setting<string>: Configuration option (default: 'value')
# machine-config-motd-feature-enabled<bool>: Enable optional feature (default: 'false')
# machine-config-motd-provider<string>: Provider-specific config (default: 'generic')
...
- objectDefinition:
    ...
    kind: ConfigurationPolicy
    metadata:
      name: machine-config-motd-csv-status
    spec:
...
3. Check policy status: `oc describe policy policy-machine-config-motd-operator-install`

### Operator Installation Issues
1. Check subscription: `oc get subscription -n machine-config-motd`
2. Check install plan: `oc get installplan -n machine-config-motd`
3. Verify operator source exists: `oc get catalogsource -n openshift-marketplace`

### Template Rendering Issues
1. Test locally: `helm template policies/machine-config-motd/`
2. Check hub escaping: Look for `{{ "{{hub" }} ... {{ "hub}}" }}` patterns
3. Validate YAML: `helm lint policies/machine-config-motd/`

## Resources
- [Operator Documentation](https://operatorhub.io/operator/machine-config-motd) - Find your operator details
