# kube-descheduler AutoShift Policy

## Overview
This policy installs the cluster-kube-descheduler-operator operator using AutoShift patterns.
...
helm template policies/kube-descheduler/
```
...
hubClusterSets:
  hub:
    labels:
      ...
      # kube-descheduler-version: 'cluster-kube-descheduler-operator.v1.x.x'  # Optional: pin to specific CSV version
...
clusters:
  my-cluster:
    labels:
      kube-descheduler: 'true'
      kube-descheduler-channel: 'fast'  # Override channel for this cluster
```
...
- name: kube-descheduler
  path: policies/kube-descheduler
  helm:
    valueFiles:
    - values.yaml
```
...
- **Version Pinning**: Add `kube-descheduler-version` label to pin to a specific CSV version
...
```yaml
kube-descheduler-version: 'cluster-kube-descheduler-operator.v1.x.x'
```

Find available CSV versions:
```bash
# List available versions for this operator
oc get packagemanifests cluster-kube-descheduler-operator -o jsonpath='{.status.channels[*].currentCSV}'
```
...
oc get crds | grep kube-descheduler
...
#### Common Patterns:
- `policy-kube-descheduler-config.yaml` - Main configuration
- `policy-kube-descheduler-<feature>.yaml` - Feature-specific configs

#### Template Structure:
```yaml
{{- $policyName := "policy-kube-descheduler-config" }}
{{- $placementName := "placement-policy-kube-descheduler-config" }}
...
spec:
  disabled: false
  dependencies:
    - name: policy-kube-descheduler-operator-install
      namespace: {{ .Values.policy_namespace }}
      apiVersion: policy.open-cluster-management.io/v1
          ...
          object-templates:
            - complianceType: musthave
              objectDefinition:
                ...
                metadata:
                  name: kube-descheduler-config
                  namespace: {{ .Values.kubeDescheduler.namespace }}
                spec:
                  # Your operator-specific configuration
                  # Use dynamic labels when needed:
                  # setting: '{{ "{{hub" }} index .ManagedClusterLabels "autoshift.io/kube-descheduler-setting" | default "default-value" {{ "hub}}" }}'
...
spec:
  ...
  predicates:
    - requiredClusterSelector:
        labelSelector:
          matchExpressions:
            - key: 'autoshift.io/kube-descheduler'
              operator: In
              values:
              - 'true'
  tolerations:
...
# kube-descheduler-setting<string>: Configuration option (default: 'value')
# kube-descheduler-feature-enabled<bool>: Enable optional feature (default: 'false')
# kube-descheduler-provider<string>: Provider-specific config (default: 'generic')
...
- objectDefinition:
    ...
    kind: ConfigurationPolicy
    metadata:
      name: kube-descheduler-csv-status
    spec:
...
3. Check policy status: `oc describe policy policy-kube-descheduler-operator-install`

### Operator Installation Issues
1. Check subscription: `oc get subscription -n openshift-kube-descheduler-operator`
2. Check install plan: `oc get installplan -n openshift-kube-descheduler-operator`
3. Verify operator source exists: `oc get catalogsource -n openshift-marketplace`

### Template Rendering Issues
1. Test locally: `helm template policies/kube-descheduler/`
2. Check hub escaping: Look for `{{ "{{hub" }} ... {{ "hub}}" }}` patterns
3. Validate YAML: `helm lint policies/kube-descheduler/`

## Resources
- [Operator Documentation](https://operatorhub.io/operator/cluster-kube-descheduler-operator) - Find your operator details
