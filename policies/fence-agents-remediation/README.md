# fence-agents-remediation AutoShift Policy

## Overview
This policy installs the fence-agents-remediation-operator operator using AutoShift patterns.
...
helm template policies/fence-agents-remediation/
```
...
hubClusterSets:
  hub:
    labels:
      ...
      fence-agents-remediation-subscription-name: 'fence-agents-remediation-operator'
      fence-agents-remediation-channel: 'stable'
      fence-agents-remediation-source: 'redhat-operators'
      fence-agents-remediation-source-namespace: 'openshift-marketplace'
      # fence-agents-remediation-version: 'fence-agents-remediation-operator.v1.x.x'  # Optional: pin to specific CSV version
...
clusters:
  my-cluster:
    labels:
      fence-agents-remediation: 'true'
      fence-agents-remediation-channel: 'fast'  # Override channel for this cluster
```
...
- name: fence-agents-remediation
  path: policies/fence-agents-remediation
  helm:
    valueFiles:
    - values.yaml
```
...
- **Version Pinning**: Add `fence-agents-remediation-version` label to pin to a specific CSV version
...
```yaml
fence-agents-remediation-version: 'fence-agents-remediation-operator.v1.x.x'
```

Find available CSV versions:
```bash
# List available versions for this operator
oc get packagemanifests fence-agents-remediation-operator -o jsonpath='{.status.channels[*].currentCSV}'
```
...
oc get crds | grep fence-agents-remediation
...
#### Common Patterns:
- `policy-fence-agents-remediation-config.yaml` - Main configuration
- `policy-fence-agents-remediation-<feature>.yaml` - Feature-specific configs

#### Template Structure:
```yaml
{{- $policyName := "policy-fence-agents-remediation-config" }}
{{- $placementName := "placement-policy-fence-agents-remediation-config" }}
...
spec:
  disabled: false
  dependencies:
    - name: policy-fence-agents-remediation-operator-install
      namespace: {{ .Values.policy_namespace }}
      apiVersion: policy.open-cluster-management.io/v1
  ...
  policy-templates:
    - objectDefinition:
        ...
        metadata:
          name: fence-agents-remediation-config
        spec:
          ...
          object-templates:
            - complianceType: musthave
              objectDefinition:
                ...
                metadata:
                  name: fence-agents-remediation-config
                  namespace: {{ .Values.fenceAgentsRemediation.namespace }}
                spec:
                  # Your operator-specific configuration
                  # Use dynamic labels when needed:
                  # setting: '{{ "{{hub" }} index .ManagedClusterLabels "autoshift.io/fence-agents-remediation-setting" | default "default-value" {{ "hub}}" }}'
...
spec:
  ...
  predicates:
    - requiredClusterSelector:
        labelSelector:
          matchExpressions:
            - key: 'autoshift.io/fence-agents-remediation'
              operator: In
              values:
              - 'true'
  tolerations:
...
# fence-agents-remediation-setting<string>: Configuration option (default: 'value')
# fence-agents-remediation-feature-enabled<bool>: Enable optional feature (default: 'false')
# fence-agents-remediation-provider<string>: Provider-specific config (default: 'generic')
...
- objectDefinition:
    ...
    kind: ConfigurationPolicy
    metadata:
      name: fence-agents-remediation-csv-status
    spec:
...
3. Check policy status: `oc describe policy policy-fence-agents-remediation-operator-install`
...
1. Test locally: `helm template policies/fence-agents-remediation/`
2. Check hub escaping: Look for `{{ "{{hub" }} ... {{ "hub}}" }}` patterns
3. Validate YAML: `helm lint policies/fence-agents-remediation/`

## Resources
- [Operator Documentation](https://operatorhub.io/operator/fence-agents-remediation-operator) - Find your operator details
